...
LOG_FILE="$HOME/.config/niri/scripts/auto-performance.log"
...
set_cpu_governor() {
        ...
        if powerprofilesctl set "$profile" 2>/dev/null; then
            ...
            if [[ "$prev_profile" != "$profile" ]]; then
                ...
                if [ -n "$DISPLAY" ] && command -v notify-send >/dev/null 2>&1; then
                    notify-send -u normal -i "power-profile" \
                        "Auto Performance" "$icon Power profile: $profile" --expire-time=3000 2>/dev/null || true
                fi
            fi
    ...
    if [ "$success" = true ]; then
        ...
        # Send notification if we're in a desktop environment
        if [ -n "$DISPLAY" ] && command -v notify-send >/dev/null 2>&1; then
            notify-send "Auto Performance" "CPU governor set to '$profile'" --expire-time=2000
        fi
...
PID_FILE="$HOME/.config/niri/scripts/auto-performance.pid"
